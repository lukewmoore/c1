...
void TransSmoothStopAtSolid(gool_object *obj, vec *velocity, zone_query *query) {
    ...
    if (being_stopped) { /* supported by a solid surface? */
        ...
        if (abs(slope_accel.x) < 10 && abs(slope_accel.y) < 10 && abs(slope_accel.z) < 10) {
            cur_velocity.x = 0;
    ...
    if (!being_stopped && (velocity->x != 0 || velocity->z != 0) && abs(delta_trans.x) < 2 && abs(delta_trans.y) < 2 && abs(delta_trans.z) < 2) {
...
static int StopAtHighestObjectBelow(gool_object *obj, vec *trans, vec *next_trans, zone_query *query) {
    ...
    for (i = 0; i < object_bound_count; i++) {
        ...
        colliding = 0;
        if ((test_y >= bound->obj->bound.p2.y || (bound->obj->process.status_b & 0x400000)) && max_y < bound->obj->bound.p2.y && TestBoundIntersection(&query->collider_bound, &bound->obj->bound)) {
            found = bound;
            query->i += 4096;
            colliding = 1;
        }
        if (colliding && (bound->obj->process.status_b & GOOL_FLAG_SOLID_TOP)) {
            ...
            exec = GetGoolObjectGlobal(bound->obj);
            header = (gool_header *)GetEntryItem(exec, 0);
            if ((!(obj->process.state_flags & GOOL_FLAG_FLING_STATE) && obj->process.invincibility_state != 5) || header->category != 0x300 || ((bound->obj->process.status_c & 0x1012) && !(bound->obj->process.state_flags & 10020))) {
                max_y = bound->obj->bound.p2.y + 1;
            }
        }
    }
    if (found) {
        GoolCollide(obj, &query->collider_bound, found->obj, &found->obj->bound);
    }
    return max_y;
}
...
static int StopAtFloor(gool_object *obj, vec *trans, vec *next_trans, zone_query *query, int *out) {
    ...
    if ((floor_nodes_y == -999999999 && solid_nodes_y == -999999999) || obj->process.vectors.velocity.y > 0) { /* not atop the floor or wall? */
...
static int StopAtCeil(gool_object *obj, vec *next_trans, zone_query *query) {
    ...
    for (i = 0; i < object_bound_count; i++) {
        bound = &object_bounds[i];
        if (!(bound->obj->process.status_b & GOOL_FLAG_SOLID_BOTTOM)) {
            continue;
        } /* skip objects which are not solid at the bottom */
        if (TestBoundIntersection(&query->collider_bound, &bound->obj->bound)) {
            found = bound;
            if (min_y == -999999999 || bound->obj->bound.p1.y <= min_y) {
                min_y = bound->obj->bound.p1.y;
            }
        }
    }
    GoolCalcBound(&test_bound_ceil, next_trans, &query->collider_bound);

    ceil = FindCeilY(obj, query, &query->nodes_bound, &query->collider_bound, 2, 1, -999999999);
    ...
    if (header->gfx.flags & 0x20000) { /* obj's zone is solid at the top? */
        ...
        if (ISERRORCODE(neighbor)) { /* no such zone? */
            ...
            rect = (zone_rect *)GetEntryItem(GetGoolObjectZone(obj), 1);
...
static int StopAtZone(gool_object *obj, vec *next_trans) {
    ...
    if (ISERRORCODE(neighbor)) { /* obj is not within the current zone or any of its neighbors? */
        ...
        // set objects zone to the correct zone
        // obj->zone = neighbor;
        SetGoolObjectZone(obj, neighbor);
    }
...
static void PlotWallB(int x1, int z1, int x2, int z2, int op) {
    ...
    z1 += 16;
    x2 += 16;
    z2 += 16; /* move range (-16,-16)-(16,16) to (0,0)-(32,32) */
    if (x2 <= 0 || x1 >= 32 || z1 >= 32) {
        return;
    } /* return if 2d range is out of bounds */
    bits = 0xFFFFFFFF;
    if (x2 < 32) {
        bits <<= (32 - x2);
    }
    if (x1 >= 0) {
        bits &= ~((int)0x80000000 >> x1);
    } /* force arithmetic shift */
    if (bits) {
        if (z1 < 0) {
            z1 = 0;
        }
...
static void PlotObjWalls(vec *next_trans, gool_object *obj, zone_query *query, int flag) {
    ...
    for (i = 0; i < object_bound_count; i++) {
        ...
        node_bound.p1.x = bound->obj->bound.p1.x + bound->obj->process.hotspot_size;
        node_bound.p1.y = bound->obj->bound.p1.y;
        node_bound.p1.z = bound->obj->bound.p1.z + bound->obj->process.hotspot_size;
        node_bound.p2.x = bound->obj->bound.p2.x - bound->obj->process.hotspot_size;
        node_bound.p2.y = bound->obj->bound.p2.y;
        node_bound.p2.z = bound->obj->bound.p2.z - bound->obj->process.hotspot_size;
        // exec = bound->obj->global;
        exec = GetGoolObjectGlobal(bound->obj);
        g_header = (gool_header *)GetEntryItem(exec, 0);
        if (zone_has_walls && (bound->obj->process.status_b & 0x10000) && ((!(obj->process.state_flags & 0x10) && obj->process.invincibility_state != 5) || g_header->category != 0x300 || ((bound->obj->process.status_c & 0x1012) && !(bound->obj->process.state_flags & 0x10020)))) {
            if (bound->obj->bound.p2.y < test_bound.p1.y || test_bound.p2.y <= bound->obj->bound.p1.y) {
                continue;
            }
        ...
        if (flag && TestBoundIntersection(&obj_bound, &bound->obj->bound)) {
            GoolCollide(obj, &obj_bound, bound->obj, &bound->obj->bound);
        }
    }
}
...
static int ReplotWalls(int op, int flags, vec *next_trans, gool_object *obj) {
    ...
    for (i = 0; i < query->result_count; i++) { /* while not done iterating results */
        ...
        if (!result->is_node) {
            ...
            level = result->level;
            type = ((node & 0xE) >> 1) + 1;
            if (type == 0 || type == 5 || (flags == 0 && type != 2) || (flags == 1 && type == 2)) {
                ++result;
                continue;
            }
            ...
            plot = ((flags == 0 && dist.p1.y <= (100 << 8) && dist.p2.y >= (-400 << 8))) || (flags == 1 && dist.p2.y >= 0) || (flags != 0 && flags != 1);
            if (!plot) {
                ++result;
                continue;
            }
...
int StopAtWalls(vec *trans, int x, int z, int *adj_x, int *adj_z, gool_object *obj, int ret) {
    ...
    while (!done) {
        ...
        for (i = 0; i < sizeof(sorted_points) / sizeof(vec28); i++) {
            ...
            ox = offs.x;
            oz = offs.y;
            if (TestWall(tx = x + ox, tz = z + oz) || TestWall(tx = x + ox, tz = z - oz) || TestWall(tx = x - ox, tz = z + oz) || TestWall(tx = x - ox, tz = z - oz)) { /* is there a wall at any offset (px,pz) s.t. abs(px)==ox, abs(pz)==oz? */
                *adj_x = tx;
                *adj_z = tz;
                return ret + 2;
            }
            if (ox == oz) {
                continue;
            } /* skip next part if coord pair does not have an inverse */
            /*
            ...
            */
            if (TestWall(tx = x + oz, tz = z + ox) || TestWall(tx = x + oz, tz = z - ox) || TestWall(tx = x - oz, tz = z + ox) || TestWall(tx = x - oz, tz = z - ox)) {
                *adj_x = tx;
                *adj_z = tz;
                return ret + 2;
            }
...
static void ZoneQueryOctreeR(
    ...
    flags = ((flag.x ? (0 <= mid.x) | ((mid.x <= (int32_t)z_rect->w) << 1) : 3) << 0) | ((flag.y ? (0 <= mid.y) | ((mid.y <= (int32_t)z_rect->h) << 1) : 3) << 2) | ((flag.z ? (0 <= mid.z) | ((mid.z <= (int32_t)z_rect->d) << 1) : 3) << 4);
    idx = 0;
...
void FindFloorY(
    ...
    for (i = 0; i < query->result_count; i++) {
        ...
        ++result;
        if (collider_bound->p2.x < nbound.p1.x || collider_bound->p2.y < nbound.p1.y || collider_bound->p2.z < nbound.p1.z) {
            continue;
        }
        ...
        if (nbound.p2.x < collider_bound->p1.x || nbound.p2.y < collider_bound->p1.y || nbound.p2.z < collider_bound->p1.z) {
            continue;
        }
...
int FindCeilY(
    ...
    for (i = 0; i < query->result_count; i++) {
        ...
        type = (node & 0xE) >> 1;
        subtype = (node & 0x3F0) >> 4;
        if (type != (type_a - 1) || type != (type_b - 1)) {
            continue;
        }
        ...
        nbound.p2.y = nbound.p1.y + (int32_t)(zone_dim.h >> min(level, max_depth.y));
        nbound.p2.z = nbound.p1.z + (int32_t)(zone_dim.d >> min(level, max_depth.z));
        if (nbound.p2.x < collider_bound->p1.x || nbound.p2.y < collider_bound->p1.y || nbound.p2.z < collider_bound->p2.z) {
            continue;
        }
        count += 1;
        sum_y1 += nbound.p1.y;
