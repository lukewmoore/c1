...
static int StopAtHighestObjectBelow(gool_object *obj, vec *trans, vec *next_trans, zone_query *query) {
  ...
  for (i=0;i<object_bound_count;i++) {
    ...
    colliding = 0;
    if ((test_y >= bound->obj->bound.p2.y || (bound->obj->process.status_b & 0x400000))
      && max_y < bound->obj->bound.p2.y
      && TestBoundIntersection(&query->collider_bound, &bound->obj->bound)) {
      found = bound;
      query->i += 4096;
      colliding = 1;
    }
...
static void PlotObjWalls(vec *next_trans, gool_object *obj, zone_query *query, int flag) {
  ...
  for (i=0;i<object_bound_count;i++) {
    bound = &object_bounds[i];
    if (flag && bound->obj == obj->process.links[6] && test_bound.p1.y >= bound->obj->bound.p2.y) { continue; }
    if (!TestBoundIntersection(&test_bound, &bound->obj->bound)) { continue; }
    ...
    if (zone_has_walls && (bound->obj->process.status_b & 0x10000)
      ...
      if (bound->obj->bound.p2.y < test_bound.p1.y || test_bound.p2.y <= bound->obj->bound.p1.y) { continue; }
...
int StopAtWalls(vec *trans, int x, int z, int *adj_x, int *adj_z, gool_object *obj, int ret) {
  ...
  while (!done) {
    ...
    collider_header = (gool_header*)obj->process.links[6]->global->items[0];
