...
uint32_t pixel_5551_8888(uint16_t p, int mode) {
    ...
    switch (mode) {
    case 0:
        a = a == 1 ? 0x7F : (r | g | b) == 0 ? 0
                                             : 0xFF;
        break;
    case 1:
        a = a == 1 ? 0xFF : (r | g | b) == 0 ? 0
                                             : 0xFF;
        break;
    case 2:
        a = a == 1 ? 0xFF : (r | g | b) == 0 ? 0
...
void TexturesUpdate() {
    ...
    for (i = 0; i < TEX_TPAGE_COUNT; i++) {
        ...
        if (texture_pages[i].eid == cache.eids[i]) {
            continue;
        }
...
static int TexturePageIdx(entry_ref *tpag) {
    ...
    if (tpag->is_eid) {
        ...
        // eid = tpag->en->eid;
        eid = GetEntryRefEntry(tpag)->eid;
    }
    for (i = 0; i < 16; i++) {
        // if (i<cache.global_count && cache.global_eids[i] == eid) { break; }
        if (texture_pages[i].eid == eid) {
            break;
        }
    }
    return i < 16 ? i : -1;
...
static tpage *TexturePage(entry_ref *tpag) {
    ...
    eid = tpag->is_eid ? tpag->eid : GetEntryRefEntry(tpag)->eid;
...
static int TextureNew(texinfo *texinfo, fvec (*uvs)[4]) {
    ...
    */
    ...
    hash = texinfo->rgninfo.color_mode << 12 | texinfo->rgninfo.segment << 10 | texinfo->rgninfo.offs_x << 5 | texinfo->rgninfo.offs_y;
    ...
    if (i == hash + TEX_CACHE_BUCKETSIZE) {
        return -1;
    }
    entry->valid = 1;
    entry->hash = hash;
