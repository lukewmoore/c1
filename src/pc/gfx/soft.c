...
#undef r
    res = 1;
    if (r_vert.x > 0x7FFF || r_vert.x < -0x8000 || r_vert.y > 0x7FFF || r_vert.y < -0x8000 || r_vert.z > 0xFFFF || r_vert.z < 0) {
        res = 0;
    }
    *out = r_vert;
    return res;
...
int SwRotTransPers(vec *in, vec *out, vec *trans, mat16 *m_rot, vec2 *offs, uint32_t proj) {
    ...
    s_vert.z = r_vert.z;
    if (s_vert.x < -0x400 || s_vert.x > 0x3FF || s_vert.y < -0x400 || s_vert.y > 0x3FF) {
        res = 0;
    }
    *out = s_vert;
    return res;
...
void SwCalcObjectRotMatrix(
    ...
    params->m_rot.m[1][0] /= 8;
    params->m_rot.m[1][0] *= -5;
    params->m_rot.m[1][1] /= 8;
    params->m_rot.m[1][1] *= -5;
    params->m_rot.m[1][2] /= 8;
    params->m_rot.m[1][2] *= -5;
...
void SwTransformSvtx(
    ...
    for (i = 0; i < header->poly_count; i++) {
        ...
        if (!info.colinfo.no_cull) {
            ndot = (r_verts[0].x * r_verts[1].y) + (r_verts[1].x * r_verts[2].y) + (r_verts[2].x * r_verts[0].y) - (r_verts[0].x * r_verts[2].y) - (r_verts[1].x * r_verts[0].y) - (r_verts[2].x * r_verts[1].y);
        ...
        if (poly->is_flat_shaded) {
            ...
            for (ii = 0; ii < 3; ii++) {
                ...
                color16.r = ((int64_t)((int64_t)(bc.r << 12)) + (cm[0][0] * ldir.x) + (cm[0][1] * ldir.y) + (cm[0][2] * ldir.z)) >> 12;
                color16.g = ((int64_t)((int64_t)(bc.g << 12)) + (cm[1][0] * ldir.x) + (cm[1][1] * ldir.y) + (cm[1][2] * ldir.z)) >> 12;
                color16.b = ((int64_t)((int64_t)(bc.b << 12)) + (cm[2][0] * ldir.x) + (cm[2][1] * ldir.y) + (cm[2][2] * ldir.z)) >> 12;
...
void SwTransformCvtx(
    ...
    for (i = 0; i < header->poly_count; i++) {
        ...
        for (ii = 0; ii < 3; ii++) {
            vert = poly_verts[ii];
            color.r = ((t1 >= 0 ? (127 - abs(t1)) * 0 + (abs(t1) * (int32_t)vert->r)
                                : (128 - abs(t1)) * 255 + ((abs(t1) - 1) * (int32_t)vert->r)) *
                       32) >>
                      (shamt + 12);
            color.g = ((t2 >= 0 ? (127 - abs(t2)) * 0 + (abs(t2) * (int32_t)vert->g)
                                : (128 - abs(t2)) * 255 + ((abs(t2) - 1) * (int32_t)vert->g)) *
                       32) >>
                      (shamt + 12);
            color.b = ((t3 >= 0 ? (127 - abs(t3)) * 0 + (abs(t3) * (int32_t)vert->b)
                                : (128 - abs(t3)) * 255 + ((abs(t3) - 1) * (int32_t)vert->b)) *
                       32) >>
                      (shamt + 12);
            prim->colors[ii] = Rgb8ToA32(color);
        }
...
void SwTransformSprite(
    ...
    info.rgninfo = info2->rgninfos[0]; // TODO: literally no idea...
    info.tpage = tpag;
    ...
    for (i = poly_id_list->len - 1; i >= 0; i--) {
        ...
        if (info.colinfo.type == 1) {
            if (poly->anim_mask) {
                ...
                info.rgninfo = wgeo_info->rgninfos[0];
            }
            info.tpage = tpag;
            texid = TextureLoad(&info, &uvs);
        } else {
...
void SwTransformZoneQuery(zone_query *query, void *ot, void **prims_tail) {
    ...
    for (i = 0; i < query->result_count; i++) {
        ...
        for (ii = 0; ii < 8; ii++) {
            if (ii % 2) {
                u_vert.x = v_nbound.p1.x;
            } else {
                u_vert.x = v_nbound.p2.x;
            }
            if ((ii / 2) % 2) {
                u_vert.y = v_nbound.p1.y;
            } else {
                u_vert.y = v_nbound.p2.y;
            }
            if ((ii / 4) % 2) {
                u_vert.z = v_nbound.p1.z;
            } else {
                u_vert.z = v_nbound.p2.z;
            }
...
void SwTransformObjectBounds(gool_bound *bounds, int count, void *ot, void **prims_tail) {
    ...
    for (i = 0; i < count; i++) {
        ...
        for (ii = 0; ii < 8; ii++) {
            if (ii % 2) {
                u_vert.x = bound->obj->bound.p1.x >> 8;
            } else {
                u_vert.x = bound->obj->bound.p2.x >> 8;
            }
            if ((ii / 2) % 2) {
                u_vert.y = bound->obj->bound.p1.y >> 8;
            } else {
                u_vert.y = bound->obj->bound.p2.y >> 8;
            }
            if ((ii / 4) % 2) {
                u_vert.z = bound->obj->bound.p1.z >> 8;
            } else {
                u_vert.z = bound->obj->bound.p2.z >> 8;
            }
            u_vert.x -= (cam_trans.x >> 8);
            u_vert.y -= (cam_trans.y >> 8);
            u_vert.z -= (cam_trans.z >> 8);
