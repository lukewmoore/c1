...
int LdatInit() {
    ...
    if (*(ns.ldat_eid) == EID_NONE) {
        ...
        header = (zone_header *)GetEntryItem(zone_spawn, 0);
        idx = header->paths_idx + ldat->path_idx_spawn;
        path_spawn = (zone_path *)GetEntryItem(zone_spawn, idx);
...
int ZdatOnLoad(entry *zone) {
    int i, idx;
    zone_header *header = (zone_header *)GetEntryItem(zone, 0);
...
static inline void ZoneTerminateDifference(entry *zone) {
    ...
    cur_header = (zone_header *)GetEntryItem(cur_zone, 0);
    ...
    for (i = 0; i < cur_header->neighbor_count; i++) {
        ...
        n_header = (zone_header *)GetEntryItem(cur_neighbor, 0);
    ...
    if (change && header->world_count) { /* change in zone, path, or pt idx? */
        ...
        if (path == cur_path &&
            ...
            if (pt_idx < mid_pt_idx) { /* new pt in first half of path? */
                raw_list = (slst_item *)GetEntryItem(slst, 0);
                ...
                raw_list = (slst_item *)GetEntryItem(slst, path->length);
        ...
        while (i != 0) { /* apply deltas from first_pt_idx to new pt */
            ...
            if (i < 0) {
                i++;
                dir = 0;
                delta = (slst_item *)GetEntryItem(slst, idx--);
            } else {
                i--;
                dir = 1;
                delta = (slst_item *)GetEntryItem(slst, ++idx);
...
void LevelSaveState(gool_object *obj, level_state *state, int flag) {
    ...
    header = (zone_header *)GetEntryItem(cur_zone, 0);
    if (header->gfx.flags & 0x2000) {
        return;
    } /* do not save state if restricted for zone */
    state->flag = flag;
    ...
    for (i = 0; i < header->path_count; i++) { /* find cur path index */
        idx = header->paths_idx + i;
        path = (zone_path *)GetEntryItem(cur_zone, idx);
        if (path == cur_path) {
            state->path_idx = i;
            break;
        }
...
void LevelRestart(level_state *state) {
    ...
    path = (zone_path *)GetEntryItem(zone, idx);
    ...
    // crash->zone = cur_zone;
    SetGoolObjectZone(crash, cur_zone);
    ...
    // }
    if ((collider = get_gool_object(crash->process.links_handle[6]))) {
        // TODO: no idea, reflection???
        collider = 0;
        crash->process.links_handle[6] = 0;
    }
    draw_count = 0;
    screen_shake = 0;
    // dword_800566E8 = 0;
...
static inline int TestPointInRect(rect *r, vec *v) {
    return (v->x >= r->loc.x << 8 && v->y >= r->loc.y << 8 && v->z >= r->loc.z << 8 && v->x <= (r->loc.x + (int32_t)r->dim.w) << 8 && v->y <= (r->loc.y + (int32_t)r->dim.h) << 8 && v->z <= (r->loc.z + (int32_t)r->dim.d) << 8);
}
...
int TestPointInBound(vec *v, bound *b) {
    return (v->x >= b->p1.x && v->y >= b->p1.y && v->z >= b->p1.z && v->x < b->p2.x && v->y < b->p2.y && v->z < b->p2.z);
}

//----- (80026C04) --------------------------------------------------------
int TestRectIntersectsBound(bound *b, rect *r) {
    return ((r->loc.x << 8) < b->p2.x && (r->loc.y << 8) < b->p2.y && (r->loc.z << 8) < b->p2.z && ((r->loc.x + (int32_t)r->dim.w) << 8) >= b->p1.x && ((r->loc.y + (int32_t)r->dim.h) << 8) >= b->p1.y && ((r->loc.z + (int32_t)r->dim.d) << 8) >= b->p1.z);
}

//----- (80026CA8) --------------------------------------------------------
int TestBoundIntersection(bound *ba, bound *bb) {
    return (bb->p2.y >= ba->p1.y && bb->p1.y < ba->p2.y && bb->p1.x < ba->p2.x && bb->p1.z < ba->p2.z && bb->p2.x >= ba->p1.x && bb->p2.z >= ba->p1.z);
}

//----- (80026D40) --------------------------------------------------------
int TestBoundInBound(bound *ba, bound *bb) {
    return (ba->p1.x >= bb->p1.x && ba->p1.y >= bb->p1.y && ba->p1.z >= bb->p1.z && ba->p2.x < bb->p2.x && ba->p2.y < bb->p2.y && ba->p2.z < bb->p2.z);
}
...
entry *ZoneFindNeighbor(entry *zone, vec *v) {
    ...
    for (i = last_idx; i >= 0; i--) { /* find the neighbor zone which contains point v */
        neighbor = NSLookup(&header->neighbors[i]);
        z_rect = (zone_rect *)GetEntryItem(neighbor, 1);
...
uint16_t ZoneFindNode(zone_rect *zone_rect, uint16_t root, rect _rect, vec *va, vec *vb, int level) {
    ...
    if (node & 1) { /* node is a leaf node? */
        ...
        if (vb->y) {
            ...
            if (ty > t || (i == 0 && abs(vb->x) < abs(vb->y)) || (i == 2 && abs(vb->z) < abs(vb->y))) {
                t = ty;
                i = 1;
            }
        }
...
uint16_t ZoneFindNearestNode(zone_rect *zone_rect, uint16_t root, rect *nrect, vec *v, int level, int flags) {
    ...
    if ((node & 1) || node == 0) { /* is the node a leaf node or empty? */
        ...
        if ((node & 1) && (!(flags & 8) || type != 3) && type != 4 && subtype != 11) { /* node is a leaf of type != 3,4, subtype != 11, and flag 4 is clear? */
            if (flags & 1) {
        ...
        ii = 0;
        ij = 0;
        ik = 0; /* child [octant] index */
        fi = 0;
        fj = 0;
        fk = 0; /* flags set when there is a subdivision */
        /* subdivide each dimension for which level is below max depth */
        if (level < tree->max_depth_x) {
            child_rect.dim.w /= 2;
            fi = 1;
...
gool_objnode ZoneFindNearestObjectNode(gool_object *obj, vec *v) {
    ...
    for (i = 0; i < object_bound_count; i++) {
        ...
        if (!(bound->obj->process.status_b & 0x20000)) {
            continue;
        }
        if (bound->obj->bound.p1.x - 20000 <= va.x && va.x <= bound->obj->bound.p2.x + 20000 /* bound box padding! */
            ...
            if (bound->obj->bound.p1.y <= va.y && va.y <= bound->obj->bound.p2.y) { /* collides with va? */
                ...
                // res.obj = bound->obj;
                res.obj_handle = store_gool_object(bound->obj);
    ...
    if (found_max && (!res.node || y_max >= v->y)) {
        ...
        res.obj_handle = store_gool_object(found_max);
    }
...
gool_objnode ZoneFindNearestObjectNode2(gool_object *obj, vec *v) {
    ...
    for (i = 0; i < object_bound_count; i++) {
        bound = &object_bounds[i];
        if (!((bound->obj->process.status_b & 0x40020000) == 0x20000)) {
            continue;
        }
        if (va.x >= bound->obj->bound.p1.x - 35000 && va.x <= bound->obj->bound.p2.x + 35000 && va.z >= bound->obj->bound.p1.z - 35000 && va.z <= bound->obj->bound.p2.z + 35000) {
    ...
    if (found_max && (!get_gool_object(res.obj_handle) || y_max >= va.y)) {
        /* found a highest below obj */
        /* and no node or max object y is nearer to va? */
        v->y = y_max;        /* replace with max object y */
        // res.obj = found_max; /* put max object in the result */
    ...
    if (!res.value) { /* has nothing been found? */
        ...
        sub_80027A08(get_gool_object(obj->process.links_handle[1]), -1, 0);
    } else if (found) { /* has an object been found? */
        // size = res.obj->process.size;
        size = get_gool_object(res.obj_handle)->process.size;
        ...
        sub_80027A08(get_gool_object(obj->process.links_handle[1]), -1, size);
    } else { /* a node was found */
        // sub_80027A08(obj->process.links[1], (res.node & 0x3C00) >> 10, 0);
        sub_80027A08(get_gool_object(obj->process.links_handle[1]), (res.node & 0x3C00) >> 10, 0);
    }
...
gool_objnode ZoneFindNearestObjectNode3(gool_object *obj, vec *v, int flags, int flag) {
    ...
    if (!(obj->process.status_b & 0x4000000) || ((flags & 4) && !(get_gool_object(obj->process.links_handle[1])->process.status_b & 0x4000000))) {
        return res;
    }
    yz_max = -999999999;
    header = (zone_header *)GetEntryItem(cur_zone, 0);
    ...
    for (i = 0; i < object_bound_count; i++) {
        ...
        if (flags & 1) {
            if (va.x >= bound->obj->bound.p1.x && va.x <= bound->obj->bound.p2.x && va.z >= bound->obj->bound.p1.z && va.z <= bound->obj->bound.p2.z) {
                if (va.y >= bound->obj->bound.p1.y && va.y <= bound->obj->bound.p2.y) {
                    // res.obj = bound->obj;
                    res.obj_handle = store_gool_object(bound->obj);
                    va.y = bound->obj->bound.p1.y;
            ...
            if (va.x >= bound->obj->bound.p1.x && va.x <= bound->obj->bound.p2.x && va.y >= bound->obj->bound.p1.y && va.y <= bound->obj->bound.p2.y) {
    ...
    if (!found && found_max) { /* a highest below or nearest z-wise object found? */
        if (flags & 1) {
            ...
            if (!get_gool_object(res.obj_handle) || yz_max >= va.y) { /* no collider found or max object y is nearer to va? */
                // res.obj = found_max;
                res.obj_handle = store_gool_object(found_max);
            ...
            if (!get_gool_object(res.obj_handle) || yz_max >= va.z) { /* no collider found or max object z is nearer to va? */
                // res.obj = found_max;
                res.obj_handle = store_gool_object(found_max);
    ...
    if (flags & 4) {
        // header = (zone_header *)GetEntryItem(GetGoolObjectZone(obj->process.links[1]), 0);
        header = (zone_header *)GetEntryItem(GetGoolObjectZone(get_gool_object(obj->process.links_handle[1])), 0);
    } else {
        header = (zone_header *)GetEntryItem(GetGoolObjectZone(obj), 0);
    }
    ...
    if (!res.value) { /* has nothing been found? */
        ...
        subtype = get_gool_object(res.obj_handle)->process.node;
    ...
    if (flags & 4) {
        ...
        ZoneColorsScaleSeek(colors, get_gool_object(obj->process.links_handle[1]), subtype, flag);
    }
    return res; /* gool object or node */
}
...
void ZoneColorsScaleSeekByEntityNode(gool_object *obj) {
    ...
    if (!get_zone_entity(obj->process.entity_handle)) {
        return;
    }
    header = (zone_header *)GetEntryItem(GetGoolObjectZone(obj), 0);
    // if (obj == crash || obj->process.links[1] == crash) {
    ...
    node = (int16_t)((get_zone_entity(obj->process.entity_handle)->spawn_flags) >> 3);
    if (node != -1) {
        subtype = (node & 0x3F0) >> 4;
        if (subtype < 39) {
            subtype = -1;
...
int ZonePathProgressToLoc(zone_path *path, int progress, gool_vectors *cam) {
    ...
    if (pt_idx == path->length - 1 && fractional) {
        for (i = 0; i < path->neighbor_path_count; i++) { /* find the following path */
            ...
            if (!(neighbor_path->relation & 2)) {
                continue;
            } /* skip preceding paths */
            ...
            n_path = (zone_path *)GetEntryItem(neighbor, n_path_idx);
            break;
        }
        if (n_path && n_path->cam_mode != 1) { /* was a following path found? */
            ...
            nz_rect = (zone_rect *)GetEntryItem(neighbor, 1);
