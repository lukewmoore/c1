...
static int CamGetProgress(vec *trans, zone_path *path, cam_info *cam, int flags, int flag) {
    ...
    if (flag) {
        if (flags & 1) {
            entrance = (int)path->entrance_index;
        }
        if (flags & 2) {
            exit = (int)path->exit_index;
        }
    }
    ...
    if (!cam->progress_made && (abs(v_dist.x) > 3200 || abs(v_dist.y) > 3200 || abs(v_dist.z) > 3200)) {
        return 0;
    }
    cam->cur_path = path;
...
static void CamFollow(gool_object *obj, uint32_t flag) {
    ...
    seek_pan = 0;
    new_pan = 0; /* seek_pan/seek_zoom boolean vals are inverted in orig impl */
    seek_zoom = 0;
    new_zoom = 0;
    ...
    for (i = 0; i < cur_path->neighbor_path_count; i++) {
        ...
        if (!(neighbor_path.relation & flags)) {
            continue;
        } /* skip the further neighbor paths */
        n_path = ZoneGetNeighborPath(cur_zone, cur_path, i);
        same_dir = (abs(cur_path->direction_x) == abs(n_path->direction_x)) && (abs(cur_path->direction_y) == abs(n_path->direction_y)) && (abs(cur_path->direction_z) == abs(n_path->direction_z));
    ...
    if (!icam) {
        return;
    } /* return if there are no cameras/potential viewpoints */
    ...
    for (i = 0; i < icam; i++) {
        ...
        if ((!cam_nearest->progress_made && cameras[i].progress_made) || ((int32_t)cameras[i].dist < dist_nearest && cam_nearest->progress_made == cameras[i].progress_made)) {
            cam_nearest = &cameras[i];
            dist_nearest = (int32_t)cam_nearest->dist;
        }
    }
    if (!cam_nearest->delta_progress) {
        return;
    }
...
int CamUpdate() {
    ...
    if (!(cur_display_flags & GOOL_FLAG_CAM_UPDATE)) {
        return 0;
    }
    ...
    case 3:
        ...
        do { /* auto-increment progress and update; or skip to next non-auto-cam path */
            ...
            if (!n_path) {
                return 1;
            } /* TODO: make ZoneGetNeighborPath return 0 if no neighbors */
    ...
    case 7:
        ...
        do {
            ...
            if (island_cam_state == -1) {
                ...
                if (abs(GoolAngDiff(point->rot_y, abs(angle))) < 0x17) {
                    break;
                }
            }
            ...
            if (n_progress >= (path_s1->length << 8)) {
                for (i = 0; i < path_s1->neighbor_path_count; i++) {
                    ...
                    if (neighbor_path.goal == next_island_cam_state || (n_path_idx == -1 && (path_s1->neighbor_path_count == 1 || (neighbor_path.goal & 3) == (next_island_cam_state & 3)))) {
                        n_path_idx = i;
                    }
                }
                if (n_path_idx == -1) {
...
int CamDeath(int *count) {
    ...
    t_header = (tgeo_header *)GetEntryItem(tgeo, 0);
