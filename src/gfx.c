...
int TgeoOnLoad(entry *tgeo) {
    ...
    if (cur_lid != LID_TITLE && cur_lid != LID_INTRO) {
        for (i = 0; i < header->texinfo_count; i++) {
            ...
            if (info->colinfo.type == 1 && (info->tpage & 1)) { // TODO: originally info->rgninfo.tpage
...
int GfxCalcObjectMatrices(svtx_frame *frame, tgeo_header *t_header,
    ...
    if (!(cur_display_flags & 0x10000)) {
        ...
        if ((status_b & 0x40000) == 0) {
            if ((int32_t)screen_proj >= r_trans->z) {
                return 0;
            }
...
void GfxTransformSvtx(svtx_frame *frame, void *ot, gool_object *obj) {
    ...
    header = (tgeo_header *)GetEntryItem(tgeo, 0);
    polys = (tgeo_polygon *)GetEntryItem(tgeo, 1);
...
void GfxTransformCvtx(cvtx_frame *frame, void *ot, gool_object *obj) {
    ...
    if (obj->process.status_b & 0x200) {
        z_header = (zone_header *)GetEntryItem(cur_zone, 0);
#ifdef PSX
...
void GfxTransformFragment(gool_frag *frag, int32_t z, eid_t tpag,
    ...
    verts[2].x = bound->p1.x;
    verts[2].y = bound->p1.y;
    verts[3].x = bound->p2.x;
    verts[3].y = bound->p1.y;
    verts[0].x = bound->p1.x;
    verts[0].y = bound->p2.y;
    verts[1].x = bound->p2.x;
    verts[1].y = bound->p2.y;
    for (i = 0; i < 4; i++) {
        verts[i].z = 0;
    ...
    info.rgninfo = frag->texinfo.rgninfos[0]; // TODO: was just rgninfo;
    info.tpage = tpag;
...
void GfxTransformFontChar(gool_object *obj, gool_glyph *glyph, int32_t z,
    ...
    info.rgninfo = glyph->texinfo.rgninfos[0];
    info.tpage = tpag;
    texid = TextureLoad(&info, &uvs);
    for (i = 0; i < 4; i++) {
        prim->verts[i] = r_verts[i];
...
void GfxLoadWorlds(zone_header *header) {
    ...
    for (i = 0; i < header->world_count; i++) {
        ...
        w_header = (wgeo_header *)GetEntryItem(wgeo, 0);
        ...
        // world->polygons=(wgeo_polygon*)GetEntryItem(wgeo, 1);
...
void GfxAnimMapPaths(uint32_t flags_a, uint32_t flags_b) {
    ...
    for (i = 0; i < header->world_count; i++) {
        ...
        if (wgeo->item_count < 4) {
            continue;
        }
        w_header = (wgeo_header *)GetEntryItem(wgeo, 0);
        id_list = (poly_id_list *)GetEntryItem(wgeo, 3);
        len = id_list->len;
        for (ii = 0; ii < len; ii++) {
            ...
            if (poly_idx >= w_header->poly_count) {
                continue;
            }
            polys = (wgeo_polygon *)GetEntryItem(wgeo, 1);
            poly = &polys[poly_idx];
            bit_idx = poly_idx2 % 32;
            mask = (1 << bit_idx);
            if ((poly_idx2 < 32 && (flags_a & mask)) || (poly_idx2 >= 32 && (flags_b & mask))) {
                poly->anim_mask = 7;
            } else {
                poly->anim_mask = 0;
            }
