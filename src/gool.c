...
gool_const_buf in_consts = { .buf = scratch.consts, .idx = 0 };  /* 80056480; gp[0x21] */
gool_const_buf out_consts = { .buf = scratch.consts, .idx = 0 }; /* 80056488; gp[0x23] */
#else
uint32_t consts[2];
gool_const_buf in_consts = { .buf = consts, .idx = 0 };  /* 80056480; gp[0x21] */
gool_const_buf out_consts = { .buf = consts, .idx = 0 }; /* 80056488; gp[0x23] */
#endif
/* .sbss */
...
int GoolObjectOrientOnPath(gool_object *obj, int progress, vec *loc) {
    ...
    if (obj->process.status_b & GOOL_FLAG_ORIENT_ON_PATH) {
        ...
        if (dir.z * (trans->x >> 8) - dir.x * (trans->z >> 8) - (loc->x >> 8) * loc_next.z + (loc->z >> 8) * loc_next.x < 0) { /* cross product */
    ...
    if ((status_a & GOOL_FLAG_TOWARD_GOAL) == (obj->process.status_a & GOOL_FLAG_TOWARD_GOAL) || (obj->process.status_a & GOOL_FLAG_CHANGE_PATH_DIR)) {
...
int GoolFindNearestObject(gool_object *obj, gool_nearest_query *query) {
    ...
    if (!((1 << category) & query->categories)) {
        return 0;
    }
    dist = ApxDist(&obj->process.vectors.trans, &query->obj->process.vectors.trans);
    if (dist >= query->dist) {
        return 0;
    } /* return if farther than current min dist */
    if (query->event == 0xFF) {   /* null event? */
        query->nearest_obj = obj; /* set as new nearest obj */
        query->dist = dist;       /* set as new min dist */
        return 0;                 /* return */
    }
    maps = (gool_state_maps *)GetEntryItem(GetGoolObjectGlobal(obj), 3);
    state_idx = maps->event_map[query->event >> 8];
    if (state_idx == 0xFF) {               /* null state index? */
        if (query->event != GOOL_EVENT_HIT /* not a hit a event? */
            || obj->process.invincibility_state == 2 || obj->process.invincibility_state == 3 || obj->process.invincibility_state == 4 || obj->process.status_c & 2) {
            if ((query->event == GOOL_EVENT_HIT_INVINCIBLE && !(obj->process.state_flags & 0x800)) || query->event == GOOL_EVENT_WIN_BOSS) { /* hit when invincible or win boss? */
                query->nearest_obj = obj;                                                                                                    /* set as new nearest obj */
                query->dist = dist;                                                                                                          /* set as new min dist */
            }
        }
    ...
    states = (gool_state *)GetEntryItem(GetGoolObjectGlobal(obj), 4);
...
gool_object *GoolObjectSpawn(entry *zone, int entity_idx) {
    ...
    if (!zone) {
        return (gool_object *)ERROR;
    };
    if (zone->type == 7) { /* zdat? */
        header = (zone_header *)GetEntryItem(zone, 0);
        idx = header->paths_idx + header->path_count + entity_idx;
        entity = (zone_entity *)GetEntryItem(zone, idx);
    } else if (zone->type == 17) { /* mdat? */
        entity = (zone_entity *)GetEntryItem(zone, 2 + entity_idx);
    } else {
        entity = (zone_entity *)GetEntryItem(zone, entity_idx);
    }
    is_main = entity->group == 3 && (entity->type == 0 || (entity->id > 0 && entity->id < 5) || (entity->type == 0x2C && entity->subtype == 0) || entity->type == 0x30);
    ...
    if (entity->type == 0x22) {                                                                                                                                                                           /* is box type? */
        if (prev_box_entity && absdiff(entity->loc.x, prev_box_entity->loc.x) < 10 && absdiff(entity->loc.z, prev_box_entity->loc.z) < 10 && absdiff(entity->loc.y, prev_box_entity->loc.y + 100) < 10) { /* last box entity within range? */
    ...
    if (zone && zone->type == 17) {
        zone = cur_zone;
    }

    // obj->zone = zone;
    SetGoolObjectZone(obj, zone);
    ...
    // exec = obj->global;
    exec = GetGoolObjectGlobal(obj);
    g_header = (gool_header *)GetEntryItem(exec, 0);
...
gool_object *GoolObjectCreate(void *parent, int exec, int subtype, int argc, uint32_t *argv, int flag) {
    ...
    header = (zone_header *)GetEntryItem(zone, 0);
    child->colors = child == crash ? header->gfx.player_colors : header->gfx.object_colors;
    return child;
}
...
int GoolObjectInit(gool_object *obj, int exec, int subtype, int argc, int *argv) {
    ...
    if (parent && parent->handle.type == 1) {
        ...
        SetGoolObjectZone(obj, GetGoolObjectZone(parent));
    ...
    if (obj->handle.subtype != 3 || *p_eid == EID_NONE || exec >= 64) {
        return ERROR;
    }
    global = NSOpen(p_eid, 1, 0);
    // obj->global = global;
    SetGoolObjectGlobal(obj, global);
...
int GoolZoneObjectTerminate(gool_object *obj, entry *zone) {
    struct entry *curr_obj_zone = GetGoolObjectZone(obj);

    if (obj->handle.type == 0 || curr_obj_zone != zone || ((obj->process.status_b & 0x1000000) && (obj->process.state_flags & 0x40000))) {
        return SUCCESS;
    }

    GoolSendEvent(0, obj, GOOL_EVENT_TERMINATE, 0, 0);
    if (curr_obj_zone == zone || (int)obj_zone == -1) {
...
#ifdef PSX
    if (anim->type != 4) {
        if ((anim->eid & 0xFF800003) != 0x80000000) {
            return;
        } /* has the eid not been resolved? i.e. is it a non-pointer? */
        res = (int)NSLookup(&anim->eid);
        if (ISERRORCODE(res)) {
            return;
        } /* return if unable to resolve */
    }
#endif
    scale_abs = *scale;
    scale_abs.x = abs(scale_abs.x); /* don't flip in x dir for negative values */
    if (anim->header.type == 1) {   /* vertex anim? */
        svtx = NSLookup(&anim->v.eid);
        frame_idx = obj->process.anim_frame >> 8;
        frame = (svtx_frame *)GetEntryItem(svtx, frame_idx);
...
int GoolObjectChangeState(gool_object *obj, uint32_t state, int argc, uint32_t *argv) {
    ...
    // exec = obj->global;
    exec = GetGoolObjectGlobal(obj);
    state_descs = (gool_state *)GetEntryItem(exec, 4);
    state_desc = &state_descs[state];
    obj->process.status_c = state_desc->status_c;
    data = (uint32_t *)GetEntryItem(exec, 2);
...
int GoolObjectUpdate(gool_object *obj, int flag) {
    ...
    if ((cur_display_flags & GOOL_FLAG_ANIMATE) == 0) {
        ...
        switch (category) {
        case 0x100:
            animate = cur_display_flags & GOOL_FLAG_ANIMATE_C1;
            break;
        case 0x300:
            animate = cur_display_flags & GOOL_FLAG_ANIMATE_C356;
            break;
        case 0x400:
            animate = cur_display_flags & GOOL_FLAG_ANIMATE_C4;
            break;
        case 0x500:
            animate = cur_display_flags & GOOL_FLAG_ANIMATE_C356;
            break;
    ...
    if (((status_b & GOOL_FLAG_FORCE_UPDATE) ||
        ...
        switch (category) {
        case 0x100:
            display = cur_display_flags & GOOL_FLAG_DISPLAY_C1;
            break;
        case 0x300:
            display = cur_display_flags & GOOL_FLAG_DISPLAY_C356;
            break;
        case 0x400:
            display = cur_display_flags & GOOL_FLAG_DISPLAY_C4;
            break;
        case 0x500:
            display = cur_display_flags & GOOL_FLAG_DISPLAY_C356;
            break;
        case 0x600:
            ...
            break;
        case 0x200:
            display = cur_display_flags & GOOL_FLAG_DISPLAY_C2;
            break;
        default:
            display = 0;
            break;
...
void GoolObjectTransform(gool_object *obj) {
    ...
    ot = context.ot;
#endif
    switch (anim->header.type) {
    case GOOL_ANIM_TYPE_VTX:
        en = NSLookup(&anim->v.eid);
        if (en->type != 20) {
            svtx = NSLookup(&anim->v.eid);
            GfxTransformSvtx((svtx_frame *)GetEntryItem(svtx, frame_idx), ot, obj);
        } else {
            cvtx = NSLookup(&anim->v.eid);
            GfxTransformCvtx((cvtx_frame *)GetEntryItem(cvtx, frame_idx), ot, obj);
        }
        if (status_b & 0x100000) {
            ...
            header = (zone_header *)GetEntryItem(zone, 0);
            obj->colors = obj == crash ? header->gfx.player_colors : header->gfx.object_colors;
        }
        break;
...
#ifdef PSX
            GfxTransformFragment(&frags[i], obj->process.size, pginfo, &bound, ot);
#else
...
static int GoolTextStringTransform(
...
#ifdef PSX
            GfxTransformFontChar(obj, glyph, obj->process.size, pginfo, &bound, prims_tail, flag);
#else
    ...
    if (center && prims_tail && font->has_backdrop) {
        ...
        bound2.p2.x += 100;
#ifdef PSX
        GfxTransformFragment(&font->backdrop, obj->process.size - 10, pginfo, &bound2, prims_tail);
#else
...
void GoolTextObjectTransform(gool_object *obj, gool_text *text, int terms_skip, void *ot, int flag) {
    ...
    start = i;
    anims = GetEntryItem(GetGoolObjectGlobal(obj), 5);
...
int GoolObjectBound(gool_object *obj) {
    ...
    if (anim->header.type == 1) { /* if svtx/cvtx, calc bounding rect from current frame */
        entry = NSLookup(&anim->v.eid);
        frame = (svtx_frame *)GetEntryItem(entry, frame_idx);
        ...
        if ((uint32_t)(rot_x - 0x200) > 0xC00) {                /* between 0 and 0x200 or 0xE00 and 0xFFF */
            bound->obj->bound.p1.x = trans.x + obj->bound.p1.x; /* i.e. 0 and 45 or 315 and 360 */
            bound->obj->bound.p1.y = trans.y + obj->bound.p1.y;
            bound->obj->bound.p1.z = trans.z + obj->bound.p1.z;
            bound->obj->bound.p2.x = trans.x + obj->bound.p2.x;
            bound->obj->bound.p2.y = trans.y + obj->bound.p2.y;
            bound->obj->bound.p2.z = trans.z + obj->bound.p2.z;
        } else if (rot_x < 0x600) {                             /* between 0x200 and 0x600 */
            bound->obj->bound.p1.x = trans.x + obj->bound.p1.z; /* i.e. 45 and 135 */
            bound->obj->bound.p1.y = trans.y + obj->bound.p1.y;
            bound->obj->bound.p1.z = trans.z - obj->bound.p2.x;
            bound->obj->bound.p2.x = trans.x + obj->bound.p2.z;
            bound->obj->bound.p2.y = trans.y + obj->bound.p2.y;
            bound->obj->bound.p2.z = trans.z - obj->bound.p1.x;
        } else if (rot_x < 0xA00) {                             /* between 0xA00 and 0xE00 */
            bound->obj->bound.p1.x = trans.x + obj->bound.p1.x; /* i.e. 225 and 315 */
            bound->obj->bound.p1.y = trans.y + obj->bound.p1.y;
            bound->obj->bound.p1.z = trans.z - obj->bound.p2.z;
            bound->obj->bound.p2.x = trans.x + obj->bound.p2.x;
            bound->obj->bound.p2.y = trans.y + obj->bound.p2.y;
            bound->obj->bound.p2.z = trans.z - obj->bound.p1.z;
        } else {                                                /* between 0x600 and 0xA00 */
            bound->obj->bound.p1.x = trans.x - obj->bound.p2.z; /* i.e. 135 and 225 */
            bound->obj->bound.p1.y = trans.y + obj->bound.p1.y;
            bound->obj->bound.p1.z = trans.z + obj->bound.p1.x;
            bound->obj->bound.p2.x = trans.x - obj->bound.p1.z;
            bound->obj->bound.p2.y = trans.y + obj->bound.p2.y;
            bound->obj->bound.p2.z = trans.z + obj->bound.p2.x;
        }
    } else {
        bound->obj->bound.p1.x = obj->process.vectors.trans.x + obj->bound.p1.x;
        bound->obj->bound.p1.y = obj->process.vectors.trans.y + obj->bound.p1.y;
        bound->obj->bound.p1.z = obj->process.vectors.trans.z + obj->bound.p1.z;
        bound->obj->bound.p2.x = obj->process.vectors.trans.x + obj->bound.p2.x;
        bound->obj->bound.p2.y = obj->process.vectors.trans.y + obj->bound.p2.y;
        bound->obj->bound.p2.z = obj->process.vectors.trans.z + obj->bound.p2.z;
    }
    if (obj->process.anim_stamp == crash->process.anim_stamp) { /* rendered during same frame as crash? */
        crash_bound.obj->bound.p1.x = crash->process.vectors.trans.x + crash->bound.p1.x;
        crash_bound.obj->bound.p1.y = crash->process.vectors.trans.y + crash->bound.p1.y;
        crash_bound.obj->bound.p1.z = crash->process.vectors.trans.z + crash->bound.p1.z;
        crash_bound.obj->bound.p2.x = crash->process.vectors.trans.x + crash->bound.p2.x;
        crash_bound.obj->bound.p2.y = crash->process.vectors.trans.y + crash->bound.p2.y;
        crash_bound.obj->bound.p2.z = crash->process.vectors.trans.z + crash->bound.p2.z;
...
static inline uint32_t *GoolTranslateGop(gool_object *obj, uint32_t gop, gool_const_buf *consts) {
    ...
    if (!(gop & 0x800)) { /* ireg or reg ref */
        idx = gop & 0x3FF;
        if (!(gop & 0x400)) {
            data = (uint32_t *)GetEntryItem(GetGoolObjectGlobal(obj), 2); /* iref ref */
        } else {
            data = (uint32_t *)GetEntryItem(GetGoolObjectExternal(obj), 2); /* reg ref */
        }
        return &data[idx];
    } else if (!(gop & 0x400)) {
...
#define dst r2.dst
    ...
    dbg = GoolObjectDebug(obj);
    if (dbg && ((dbg->flags & GOOL_FLAGS_PAUSED) == GOOL_FLAGS_PAUSED)) {
        return SUCCESS;
    } /* paused */
#endif
    while (1) {
        ...
        case 0x26:
            ...
            if (G_OPCODE(instruction) == 0x26) {
                ...
                if (src) {
                    GoolObjectPush(obj, (uint32_t)src);
                }
            } else {
                if (!dst) {
                    break;
        ...
        case 0x27: {
            ...
            G_TRANS_GOPA(obj, instruction, a);
            G_TRANS_GOPB_OUT(obj, instruction, dst);
            anim_data = GetEntryItem(GetGoolObjectGlobal(obj), 5);
...
int GoolObjectTryBreak(gool_object *obj, uint32_t opcode, uint32_t flags, int res) {
    ...
    if ((flags & GOOL_FLAGS_TRANS_BLOCK) == GOOL_FLAGS_TRANS_BLOCK) { /* trans block currently executing? */
        if ((dbg->flags & GOOL_FLAG_STEP_TRANS) && (!(dbg->flags & GOOL_FLAG_SERIAL_STEP) || !(dbg->flags & GOOL_FLAG_SERIAL_STATE))) {
            brk = 1;
    ...
    if ((flags & GOOL_FLAGS_CODE_BLOCK) == GOOL_FLAGS_CODE_BLOCK) {
        if ((dbg->flags & GOOL_FLAG_STEP_CODE) && (!(dbg->flags & GOOL_FLAG_SERIAL_STEP) || (dbg->flags & GOOL_FLAG_SERIAL_STATE))) {
            brk = 2;
            if (rtn && (dbg->flags & GOOL_FLAG_SERIAL_STEP)) {
                dbg->flags &= ~GOOL_FLAG_SERIAL_STATE;
            }
...
static inline void GoolOp2225unnamed(gool_object *obj, uint32_t instruction) {
...
static inline void GoolOpMisc(gool_object *obj, uint32_t instruction) {
    uint32_t sop1, *ptr;
    union {
...
#define idx r2.idx
    ...
    case 12: /* misc */
        ...
        case 4: { /* set object zone */
            ...
            /* change zone */
            // dst_obj->zone = obj_zone;
            SetGoolObjectZone(dst_obj, obj_zone);
            break;
        }
        case 5:
...
static inline int GoolOpChangeAnim(gool_object *obj, uint32_t instruction, uint32_t *flags) {
    gool_anim *anim;
...
static inline int GoolOpChangeAnimFrame(gool_object *obj, uint32_t instruction, uint32_t *flags) {
...
static inline void GoolOpTransformVectors(gool_object *obj, uint32_t instruction) {
    uint32_t sop, *ptr;
    ...
    case 6: { /* transform obj [model] vertex */
        ...
        header = (tgeo_header *)GetEntryItem(tgeo, 0);
...
static inline void GoolOpJumpAndLink(gool_object *obj, uint32_t instruction, uint32_t *flags) {
...
static inline int GoolOpSendEvent(gool_object *obj, uint32_t instruction, uint32_t *flags, gool_object *recipient, uint32_t opcode) {
...
static inline int GoolOpReturnStateTransition(gool_object *obj,
...
int GoolSendEvent(gool_object *sender, gool_object *recipient, uint32_t event, int argc, uint32_t *argv) {
    ...
    if (state != 0xFF) { /* state is non-null? */
        ...
        // exec = recipient->global;
        exec = GetGoolObjectGlobal(recipient);
        descs = (gool_state *)GetEntryItem(exec, 4);
        desc = &descs[state];
        if ((recipient == crash) &&
...
int GoolSendIfColliding(gool_object *recipient, gool_event_query *query) {
    ...
    switch (query->type) {
    case 4:
        header = (gool_header *)GetEntryItem(GetGoolObjectGlobal(recipient), 0);
